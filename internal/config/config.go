...
// DiscoverServerExe looks for a bundled llama-server beside this binary and
// returns its path, or "" if there is none.
//
// Split out of HealServerExe because the two questions are different, and
// conflating them cost every Linux user their first run. HealServerExe asks
// "this configured path is broken, is there a good one?" and deliberately
// declines to answer when server_exe is empty — because empty is not a mistake,
// it is how remote mode is spelled. `gobbonet config set server_exe ""` is
// exactly what the Windows installer runs to select a remote backend.
//
// This asks a question with no such baggage: "did this install ship an engine?"
// That is a property of the install, not of the user's configuration, so the
// answer is safe to show someone who has not chosen a backend yet without
// writing anything down. `gobbonet setup` uses it to decide whether to offer
// the local option at all; it had no fallback before and simply told everyone
// there was no engine, while one sat unopened beside the binary (SETUP-1).
//
// Nothing here writes to config. Choosing remote still leaves server_exe empty.
func DiscoverServerExe() string {
	name := serverExeName()
	var dirs []string
	if exe, err := os.Executable(); err == nil {
	...
	for _, dir := range dirs {
		...
		// sibling is the portable-unzip layout. llama-cpp-cpu/ is the Debian
		// package's fallback build, which is the only engine present on a
		// machine where the Vulkan one was not bundled.
		for _, candidate := range []string{
			filepath.Join(dir, "llama-cpp", name),
			filepath.Join(dir, "llama-cpp-cpu", name),
...
//
// All of that is a Windows story. The scripts named above are the only readers
// anywhere, and they are batch files. Nothing on Linux reads the sidecar, so
// nothing on Linux writes one — see PortFileSupported.
// ---------------------------------------------------------------------------

// PortFileSupported reports whether this platform uses the sidecar at all.
//
// Windows only, and that is not an accident of where the file happens to be
// writable — it is what the file is for. The only readers are setup-lan.bat and
// launch.bat, neither of which ships in the .deb. The Linux launcher was
// written the other way round and says so at gobbonet-launch:187: "The port is
// a config key, not a sidecar file." It asks `gobbonet config get listen_port`.
//
// Writing it anyway on Linux was worse than useless (NEW-1). The path derives
// from the executable's directory, which on Windows is the install folder and
// writable, and on Linux is /usr/lib/gobbonet — root-owned, and deliberately so
// per installer-linux/README.md. So every launch by every normal user on every
// Linux install printed a permission warning about a file that platform has no
// reader for. The write could never succeed and nothing wanted it to.
//
// Skipping it here rather than relocating it is the reason the warning, the
// doctor row and the setup-lan.bat mention (NEW-2, NEW-3) all resolve at once:
// there is no longer a Linux sidecar to be wrong about.
func PortFileSupported() bool { return runtime.GOOS == "windows" }
...
//
// Empty on platforms that do not use the sidecar, and empty is a valid answer
// here rather than a failure — see PortFileSupported.
func PortFilePath() string {
	if !PortFileSupported() {
		return ""
	}
...
//
// A no-op returning nil where the sidecar is not used. Callers treat a non-nil
// error as something worth telling the user about, so "this platform has no
// sidecar" must not be spelled as an error — that is exactly the alarming
// non-problem NEW-1 was about.
//
// Still best effort where it does apply: an install folder can be read-only
// too, and a server that is bound and serving must not die over a hint file.
func WritePortFile(port int) error {
	...
	return writePortFileTo(path, port)
}
...
func ReadPortFile() int {
	...
	return readPortFileAt(path)
}

// writePortFileTo and readPortFileAt hold the actual format, split from the
// platform policy above so the round trip stays under test everywhere rather
// than skipping on the platforms that do not ship the sidecar.
func writePortFileTo(path string, port int) error {
	return os.WriteFile(path, []byte(strconv.Itoa(port)+"\n"), 0o644)
}

func readPortFileAt(path string) int {
	raw, err := os.ReadFile(path)
	if err != nil {
		return 0
	}
...
// PortFileWritable answers whether the sidecar could be written right now, by
// trying it rather than by reading permission bits.
//
// doctor needs this to tell "not written yet" apart from "can never be
// written" (NEW-2). Mode bits are not enough to decide that: ACLs, read-only
// mounts and container filesystems all deny writes that the bits appear to
// allow. The only honest test is to attempt one, so this creates a temporary
// file in the target directory and removes it.
//
// Reports (false, nil) where the platform has no sidecar — nothing is wrong,
// there is simply nothing to write.
func PortFileWritable() (bool, error) {
	path := PortFilePath()
	if path == "" {
		return false, nil
	}
...
// dirWritable answers by writing, then cleaning up after itself.
//
// Split out so the probe itself stays under test on platforms that have no
// sidecar for PortFileWritable to ask about.
func dirWritable(dir string) (bool, error) {
	f, err := os.CreateTemp(dir, ".gobbonet-port-probe-*")
	if err != nil {
		return false, err
	}
	name := f.Name()
	...
	if err := os.Remove(name); err != nil {
		// Left behind a stray dotfile in the install directory. Say so rather
		// than reporting a clean success: a diagnostic that litters is worth
		// knowing about.
		return true, err
	}
	return true, nil
