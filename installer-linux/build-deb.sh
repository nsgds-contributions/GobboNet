...
# A distributed source tree has no .git, and that used to be fatal: the build
# died here with "not a git repository" before producing anything, so nobody
# working from a tarball or an exported zip could build a package at all
# (PKG-5). The commit is provenance, not a prerequisite.
if SHA="$(cd "$ROOT" && git rev-parse --short HEAD 2>/dev/null)" && [ -n "$SHA" ]; then
    ...
    # A date rather than a stand-in hash. "nogit" plus the day says honestly
    # that the source could not be identified; a fabricated hash would claim it
    # had been. SOURCE_DATE_EPOCH is honoured so reproducible builds stay so.
...
# ---------------------------------------------------------------------------
glibc_floor() {
    # Only inspect engines that were actually bundled. This used to name the
    # CPU engine unconditionally, but that directory exists only when
    # BUNDLE_CPU_ENGINE=1 -- so on the DEFAULT path objdump exited non-zero,
    # pipefail carried it out of the command substitution, set -e killed the
    # build, and 2>/dev/null meant it did so without printing anything at all.
    # A silent exit 1 immediately after "GPU engine carries libggml-vulkan.so
    # -- ok" is not a diagnosable failure.
    local targets=("$VENDOR/llama-cpp/llama-server")
    [ "$BUNDLE_CPU" = "1" ] && targets+=("$VENDOR/llama-cpp-cpu/llama-server")
    objdump -T "${targets[@]}" 2>/dev/null \
